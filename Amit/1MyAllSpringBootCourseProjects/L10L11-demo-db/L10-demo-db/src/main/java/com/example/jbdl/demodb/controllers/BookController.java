...
//    }
    ...
    @GetMapping("/book") //first push in db then get from db  //localhost:8080/book?id=2
    ...
    public Book getBook(@RequestParam("id") int id) throws SQLException {
        //Book book = new Book(1, "Python", "Jim", 1000);//no error due to AllArgsConstructor
...
//                           @RequestParam("author") String authorName, @RequestParam("cost") int cost){
      public void insertBook(@Valid @RequestBody BookCreateRequest bookCreateRequest) throws SQLException {
        //Book book = new Book(id, name, authorName,cost);//using AllArgsConstructor
//        Book book = new Book();
//        book.setId(id);
//        book.setName(name);
//        book.setAuthorName(authorName);
//        book.setCost(cost);
        //not using AllArgsConstructor but using Setter each time..instead of doing this way, we can use
        //builder().. internally calling setters, can write in one line(it will automatically use new keyword as well
        //Now @Builder requires @AllArgsConstructor.. if we remove this constructor.. @Builder give error
        //Book book = Book.builder().id(id).name(name).authorName(authorName).cost(cost).build();
